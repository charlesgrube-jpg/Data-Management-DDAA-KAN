...
# Skip old pipeline.models - they don't support SSL features
# Always use inline models that support input_dim parameter
get_detector = None
print("[INFO] Using inline model definitions for SSL feature support")
...
# -----------------------------------------------------------------------------
# KAN Linear Layer (B-Splines) - Ported from train_kan.py
# -----------------------------------------------------------------------------
class KANLinear(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        bsplines = self.b_splines(x)
        spline_output = F.linear(
            bsplines.view(x.size(0), -1),
            (self.spline_weight * (self.spline_scaler.unsqueeze(-1) if self.enable_standalone_scale_spline else 1.0)).view(self.out_features, -1),
        )
        return base_output + spline_output

# -----------------------------------------------------------------------------
# Physics Loss - Ported from train_pinn.py
# -----------------------------------------------------------------------------
class TemporalSmoothnessLoss(nn.Module):
    ...
    def forward(self, features):
        """
        features: (Batch, Time, Dim) or (Batch, Dim) - If just dim, cannot compute temporal physics.
        """
        # If input is (Batch, Dim) [e.g. SSL embeddings], check if we can reshape or if it's already pooled
        if features.dim() == 2:
...
class KANDetector(BaseDetector):
    """
    SOTA KAN-based deepfake detector (B-Splines).
    Includes Physics-Informed Loss support.
    """
    
    def __init__(self, n_bins=84, hidden_dim=256, hidden_dims=None, num_classes=2, input_dim=None, physics_weight=0.0, **kwargs):
        super().__init__()
        self.input_dim = input_dim or (n_bins * 100)
        self.physics_weight = physics_weight
        
        if hidden_dims is None:
            # Default KAN architecture
            hidden_dims = [hidden_dim, hidden_dim // 2]
        
        layers = []
        in_dim = self.input_dim
        
        # BN/LayerNorm at input is crucial for KAN stability
        layers.append(nn.LayerNorm(in_dim))
        
        for h_dim in hidden_dims:
            layers.append(KANLinear(in_dim, h_dim))
            # KANLinear includes activation
            # layers.append(nn.LayerNorm(h_dim)) # Optional but good for depth
            in_dim = h_dim
        
        self.backbone = nn.Sequential(*layers)
        self.classifier = KANLinear(hidden_dims[-1], num_classes)
    ...
    def training_step(self, x, y, criterion):
        """
        ...
        # Physics Loss (Latent Smoothness)
        # We need the sequence for physics, but if x is already flattened/pooled, we skip
        # Note: SSL bundles are usually (B, 768) [Mean Pooled?]. 
        # If they are (B, T, 768), we can use physics.
        phys_loss = torch.tensor(0.0, device=x.device)
        if self.physics_weight > 0 and x.dim() == 3:
             # Calculate on raw sequence input if available
             phys_loss = self.phys_loss_fn(x) * self.physics_weight
             
        loss = ce_loss + phys_loss
        return loss, {"ce": ce_loss.item(), "phys": phys_loss.item()}
...
def main():
    ...
    parser.add_argument("--physics_weight", type=float, default=0.0, 
                       help="Weight for physics-informed consistency loss (default: 0.0)")
    ...
    if args.model == "transformer":
        ...
        model_kwargs["hidden_dims"] = [args.hidden_dim, args.hidden_dim // 2]
        model_kwargs["physics_weight"] = args.physics_weight
    ...
    if args.physics_weight > 0:
        print(f"Physics-Informed Loss Enabled (Weight: {args.physics_weight})")
    ...
    use_physics = (args.model == "pinn" or args.physics_weight > 0)
